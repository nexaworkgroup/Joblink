...
import {
  ...
  User, LogOut, Globe, Users, PlusCircle, BarChart2,
  ChevronLeft, Menu, Sun, Moon
} from 'lucide-react'
...
export default function AppLayout() {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-4 border-b flex-shrink-0" style={{borderColor:'var(--border)'}}>
        <Link to={isSeeker?'/dashboard':'/employer/dashboard'} className="flex items-center gap-2 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-xl bg-primary flex items-center justify-center flex-shrink-0 text-white font-bold text-sm">J</div>
          {(!collapsed||mobile) && <span className="font-bold text-[15px] truncate gradient-text">JobLink</span>}
        </Link>
        {!mobile && (
          <button onClick={()=>setCollapsed(c=>!c)} className="p-1 rounded-lg hover:bg-[var(--border-soft)] text-[var(--text-2)] hidden md:flex">
        ...
        {navItems.map(({to, icon:Icon, label}) => {
          ...
          return (
            <Link key={to} to={to} className={clsx(
              ...
              active ? 'bg-primary text-white' : 'text-[var(--text-2)] hover:bg-[var(--border-soft)] hover:text-[var(--text-1)]'
            )}>
          ...
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl" style={{background:'var(--border-soft)'}}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px]" style={{color:'var(--text-3)'}}>
                {/* Credits badge for seekers */}
                {isSeeker ? `${user?.credits_balance||0} credits` : 'Employer'}
              </p>
            </div>
          </div>
        )}
        <div className={clsx('flex items-center gap-2 px-1', collapsed&&!mobile?'flex-col':'')}>
          <button onClick={toggle} className={clsx('relative w-10 h-5 rounded-full transition-colors flex-shrink-0', dark?'bg-primary':'bg-gray-200')}>
          ...
          <button onClick={signOut} className={clsx('flex items-center gap-1.5 text-xs transition-colors px-2 py-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500', collapsed&&!mobile?'justify-center w-full ml-0':'ml-auto')} style={{color:'var(--text-2)'}}>
            <LogOut size={13}/>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{background:'var(--bg)'}}>
      <aside className={clsx('hidden md:flex flex-col border-r transition-[width] duration-300 flex-shrink-0', collapsed?'w-[58px]':'w-[216px]')} style={{background:'var(--surface)',borderColor:'var(--border)'}}>
        <NavContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-[260px] z-50 flex flex-col animate-slide-up" style={{background:'var(--surface)',borderRight:'1px solid var(--border)'}}>
            <NavContent mobile />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <nav className="md:hidden flex items-stretch border-t flex-shrink-0" style={{background:'var(--surface)',borderColor:'var(--border)',paddingBottom:'env(safe-area-inset-bottom,0px)'}}>
          {navItems.slice(0,5).map(({to,icon:Icon,label}) => {
            const active = location.pathname===to||location.pathname.startsWith(to+'/')
            return (
              <Link key={to} to={to} className={clsx('flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 transition-colors', active?'text-primary':'text-[var(--text-3)]')}>
                <Icon size={20}/>
